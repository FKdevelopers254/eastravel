...
class HomeScreenSafari extends StatefulWidget {
  const HomeScreenSafari({Key? key}) : super(key: key);

  @override
  State<HomeScreenSafari> createState() => _HomeScreenSafariState();
}

class _HomeScreenSafariState extends State<HomeScreenSafari> with SingleTickerProviderStateMixin {
        ...
        children: <Widget>[
          ...
          TabBar(
            ...
            tabs: const [
              ...
              Tab(
                child: Text(
                  'Safari Amenities',style: TextStyle(
                  fontFamily: 'Montserrat',
                  fontSize: 15.0,
        ...
        builder: (context, snapshot) {
          ...
          return Column(
            ...
            children: snapshot.data!.docs.map((document) {
              ...
              final address = document.get('location');
  ...
  void initState() {
    ...
    firestoreInstance
      ...
      if (querySnapshot.size > 0) {
        ...
        setState(() {
          _wifiEnabled = data['gst'] ?? false;
          _barEnabled = data['unlimitedmilage'] ?? false;
          _parkingEnabled = data['alpef'] ?? false;
          _spaEnabled = data['esg'] ?? false;
          _poolEnabled = data['FullBoardAccomodation'] ?? false;
          _restaurantEnabled = data['airporttransfers'] ?? false;
        });
      }
    });
  }
  ...
  void _onWifiChanged(bool value) {
    ...
    firestoreInstance
      ...
      if (querySnapshot.size > 0) {
        ...
        firestoreInstance.collection('safariamenities').add({
          'email': user!.email,
          'gst': value,
          'unlimitedmilage': _barEnabled,
          'alpef': _parkingEnabled,
          'esg': _spaEnabled,
          'FullBoardAccomodation': _poolEnabled,
          'airporttransfers': _restaurantEnabled,
        });
      }
    });
  }
    ...
    firestoreInstance
      ...
      if (querySnapshot.size > 0) {
        ...
        firestoreInstance
            .collection('safariamenities')
            .doc(documentId)
            .update({'unlimitedmilage': value});
      } else {
        firestoreInstance.collection('safariamenities').add({

          'email': user!.email,
          'gst': _wifiEnabled,
          'unlimitedmilage': value,
          'alpef': _parkingEnabled,
          'esg': _spaEnabled,
          'FullBoardAccomodation': _poolEnabled,
          'airporttransfers': _restaurantEnabled,
    ...
    firestoreInstance
      ...
      if (querySnapshot.size > 0) {
        ...
        firestoreInstance
            .collection('safariamenities')
            .doc(documentId)
            .update({'FullBoardAccomodation': value});
      } else {
        firestoreInstance.collection('safariamenities').add({
          'email': user!.email,
          'gst': _wifiEnabled,
  ...
  Widget build(BuildContext context) {
    return  Container(
      child: Column(
        children: [
          SwitchListTile(
              title: Text('Government statutory taxes'),
            value: _wifiEnabled,
            onChanged: _onWifiChanged,
          ),
          SwitchListTile(
            title: Text('Unlimited mileage on game viewing drives'),
            value: _barEnabled,
            onChanged: _onBarChanged,
          ),
          SwitchListTile(
            title: Text('All park entrance fees'),
          ...
          SwitchListTile(
            title: Text('Professional English-speaking guide driver'),
            value: _spaEnabled,
            onChanged: _onSpaChanged,
          ),
          SwitchListTile(
            title: Text('Full board accommodation while on safari'),
            value: _poolEnabled,
            onChanged: _onPoolChanged,
          ),
          SwitchListTile(
            title: Text('Arrival and departure Airport transfers'),
